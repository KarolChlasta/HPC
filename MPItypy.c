...
int main(int argc, char **argv)
{
	...
	if(rank != 0)
	{
		strcpy(tabrecieved.name, "Piotr");
		tabrecieved.age = 14;
		printf("W procesie %d mam na imie %s i mam %d lat\n",rank,tabrecieved.name,tabrecieved.age);		
		MPI_Recv(&tabrecieved, 1, new_type, 0, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		printf("W procesies %d otrzymalem imie: %s ",rank,tabrecieved.name);
		printf("oraz wiek: %d\n",tabrecieved.age);
	}
	else
	{
		strcpy(tabrecord.name, "Jan");
		tabrecord.age = 51;
		for (r=1; r<size; r++)
